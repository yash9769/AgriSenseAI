...
const SECTORS: Sector[] = [
  { id: 'sec-a', name: 'Sector A', crop: 'Wheat', status: 'healthy', moisture: '38%', yieldEstimate: '4.2 tons/ha', coordinates: 'clip-path-a' },
  { id: 'sec-b', name: 'Sector B', crop: 'Corn', status: 'critical', moisture: '22%', yieldEstimate: '8.5 tons/ha (At Risk)', coordinates: 'clip-path-b' },
  { id: 'sec-c', name: 'Sector C', crop: 'Soybeans', status: 'healthy', moisture: '41%', yieldEstimate: '3.1 tons/ha', coordinates: 'clip-path-c' },
  { id: 'sec-d', name: 'Sector D', crop: 'Potatoes', status: 'warning', moisture: '30%', yieldEstimate: '35 tons/ha', coordinates: 'clip-path-d' },
];
...
export const FieldMapScreen = ({ setScreen }: { setScreen: (s: Screen) => void }) => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-surface-container-lowest">
      <TopBar title="Interactive Field Map" activeScreen="field-map" setScreen={setScreen} />
                ...
                {SECTORS.map((sector) => (
                    ...
                    >
                        ...
                        <div className="relative z-10 flex flex-col items-center">
                            <span className="font-headline font-bold text-xl text-emerald-950">{sector.name}</span>
                            <span className="text-xs font-semibold text-on-surface-variant uppercase tracking-widest">{sector.crop}</span>
                        </div>
            ...
            >
              <div>
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-xs font-bold uppercase tracking-widest text-on-surface-variant">
                        {activeSector.status === 'healthy' ? "Optimal Condition" : 
                         activeSector.status === 'warning' ? "Requires Attention" : "Critical Alert"}
                    </span>
                </div>
                <h2 className="text-4xl font-headline font-black text-primary tracking-tight">{activeSector.name}</h2>
                <div className="mt-4 flex items-center justify-between border-y border-outline-variant/20 py-4">
                    <div className="flex flex-col">
                        <span className="text-xs uppercase tracking-widest text-outline">Crop Setup</span>
                        <span className="font-bold text-lg text-emerald-900">{activeSector.crop}</span>
                    </div>
                    <div className="w-px h-8 bg-outline-variant/30"></div>
                     <div className="flex flex-col items-end">
                        <span className="text-xs uppercase tracking-widest text-outline">Area</span>
              ...
              <div className="space-y-4">
                  <div className="bg-surface-container-lowest p-4 rounded-2xl flex items-center gap-4 border border-outline-variant/10">
                      ...
                      <div className="flex-1">
                          <p className="text-xs font-bold text-on-surface-variant uppercase tracking-widest">Soil Moisture</p>
                  ...
                  <div className="bg-surface-container-lowest p-4 rounded-2xl flex items-center gap-4 border border-outline-variant/10">
                      ...
                      <div className="flex-1">
                          <p className="text-xs font-bold text-on-surface-variant uppercase tracking-widest">Est. Yield</p>
              ...
              {activeSector.status === 'critical' ? (
                  <div className="bg-error-container/50 border border-error/20 p-5 rounded-2xl flex flex-col gap-4">
                      ...
                      <p className="text-sm text-on-surface-variant">Low moisture combined with high temperature is triggering severe heat stress.</p>
                      <button 
                  ...
                  <div className="mt-auto pt-4">
                      ...
                      >
                          ...
                          <span className="text-[10px] font-normal uppercase tracking-widest opacity-70">On {activeSector.name}</span>
                      </button>
                  </div>
              )}
            </motion.div>
