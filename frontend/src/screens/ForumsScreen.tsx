...
export const ForumsScreen = ({ setScreen }: { setScreen: (s: Screen) => void }) => {
  ...
  const handleCreatePost = () => {
    ...
    fetch(`/api/social/posts?title=${encodeURIComponent(newPost.title)}&content=Empty&author_id=1`, { 
        method: 'POST' 
    })
      .then(res => res.json())
      .then(post => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      <TopBar title="Forums" setScreen={setScreen} activeScreen="forums" />
      
      <div className="flex-1 overflow-y-auto p-8 max-w-7xl mx-auto w-full scrollbar-hide">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
          <div>
            <h2 className="text-4xl font-headline font-extrabold text-primary tracking-tight">Community Forums</h2>
            <p className="text-on-surface-variant max-w-xl mt-2">Connect with other farmers, share experiences, and solve agricultural challenges together.</p>
          </div>
          <button 
        ...
        <div className="relative w-full max-w-2xl mb-12 group">
          ...
          <div className="relative flex items-center bg-surface-container-low border border-emerald-900/5 rounded-full pl-6 pr-4 py-1">
             ...
             <input 
                ...
                placeholder="Search by topic, crops, or pests..."
        ...
        <div className="grid grid-cols-1 gap-6">
              ...
              >
                ...
                <div className="flex items-center gap-4 self-end md:self-center">
                    <div className="flex flex-col items-center p-3 rounded-xl bg-surface-container/50">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-tighter opacity-50">Replies</span>
                    </div>
          ...
          {filteredPosts.length === 0 && (
            <div className="text-center py-20 bg-surface-container-low/50 rounded-[3rem] border border-dashed border-outline-variant/20">
                ...
                <p className="text-on-surface-variant font-medium">No discussions found matching your search.</p>
            </div>
          )}
        </div>
      </div>
        ...
        {isPosting && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              ...
              <h3 className="text-3xl font-headline font-extrabold text-primary mb-2">New Topic</h3>
              <p className="text-on-surface-variant mb-8 font-medium">Share your challenge or advice with the guild.</p>

              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-outline ml-1">Title</label>
                  <input 
                    type="text" 
                    value={newPost.title}
                    onChange={(e) => setNewPost({...newPost, title: e.target.value})}
                    placeholder="e.g. Best fertilizer for Sector A-1"
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-outline ml-1">Tags (Comma separated)</label>
                  <input 
                    type="text" 
                    value={newPost.tags}
                    onChange={(e) => setNewPost({...newPost, tags: e.target.value})}
                    placeholder="tomato, help, disease"
                    className="w-full bg-surface-container-low border-none rounded-2xl p-5 text-sm focus:ring-2 focus:ring-primary/20 outline-none"
                  />
                </div>
                <button 
